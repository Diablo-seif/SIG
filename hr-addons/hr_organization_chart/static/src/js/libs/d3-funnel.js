(function(global) {
     ...
     */
    function D3Funnel(data, options, counts) {
        ...
        for (var i = 0; i < this.data.length; i++) {
            ...
            if (typeof this.data[i][4] === "undefined" ||
                !hexExpression.test(this.data[i][4])) {
                this.data[i][4] = colorScale(i);
            } // End if
        ...
        this.onClickBlock = settings.onClickBlock; 
    ...
    D3Funnel.prototype.draw = function(selector) {
        ...
        for (var i = 0; i < sectionPaths.length; i++) {
            ...
            var data = {
                ...
                label: this.data[i][0],
                value: this.data[i][1],
                domain: this.data[i][2],
                object: this.data[i][3],
                baseColor: this.data[i][4],
                fill: fill
            ...
            group = svg.append("g");
            group.append("svg:title").text(this.counts[i]);
            ...
            // Add the hover events
            if (this.onClickBlock) {
                path.on("click", this.onClickBlock)
            } // End if
            // Add the section label
            var textStr = this.data[i][0] + ": " + this.data[i][1];
            var textX = this.width / 2; // Center the 30text
            var textY = !this.isCurved ? // Average height of bases
                ((this.dy * (2 * i + 1)) / 2) + 25 :
                ((paths[1][1] + paths[3][1]) / 2) + 25;
    ...
    D3Funnel.prototype._drawTopOval = function(svg, sectionPaths) {
        ...
        // Draw top oval
        svg.append("path")
            .attr("fill", shadeColor(this.data[0][4], -0.4))
            .attr("d", path);
