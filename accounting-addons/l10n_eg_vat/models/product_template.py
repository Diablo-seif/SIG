...
class ProductTemplate(models.Model):
    ...
    account_vat_tax_types_in_ids = fields.Many2many(
        'account.vat.tax.types',
        'account_vat_tax_types_in',
        'account_move_line_id',
        'account_vat_tax_types_id',
        string="Product Type (Vendor VAT)"
    )
    account_vat_tax_types_out_ids = fields.Many2many(
        'account.vat.tax.types',
        'account_vat_tax_types_ou',
        'account_move_line_id',
        'account_vat_tax_types_id',
        string="Product Type (Customer VAT")
    account_vat_tax_types_in_domain_vat = fields.Char(
        compute='_compute_tax_domain',
        store=True,
    )
    account_vat_tax_types_in_domain_table = fields.Char(
        compute='_compute_tax_domain',
        store=True
    )
    account_vat_tax_types_out_domain_vat = fields.Char(
        compute='_compute_tax_domain',
        store=True
    )
    account_vat_tax_types_out_domain_table = fields.Char(
        compute='_compute_tax_domain',
        store=True,
    )
